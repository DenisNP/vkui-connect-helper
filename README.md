...
`apiVersion` | _string_ | Версия API ВКонтакте для передачи в вызовы методов. | нет | `5.95`
...
`MODE_AUTO` | Настройка по-умолчанию, программа вызовет событие `VKWebAppInit` и будет ждать ответа в течение `initWaitTime` миллисекунд (по-умолчанию 1000). Если ответ поступит, произойдёт переключение в `MODE_PROD`, иначе в `MODE_DEV`. Нельзя использовать этот режим в продакшене, потому что он требует наличия токена, которого не должно быть в коде в продакшен-версии.
