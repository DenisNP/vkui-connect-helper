...
`mode` | _MODE_PROD, MODE_AUTO, MODE_DEV_ | Режим работы библиотеки, смотрите ниже в примере инициализации. **Не используйте в продакшене режим AUTO!** | **да** но только в продакшене | `MODE_AUTO`
...
Режим | Описание
--- | ---
`MODE_DEV` | Режим, в котором библиотека будет считать, что модуля **VKUIConnect** нет, все запросы будут напрямую через API, а неподдерживаемые события не будут работать.
`MODE_PROD` | Режим, в котором библиотека будет все события пытаться отправлять через VKUIConnect. Если он не подключен, программа перестанет выполняться в этом месте. Собственно, это поведени и вдохновило меня на написание **vkui-connect-helper**.
`MODE_AUTO` | Настройка по-умолчанию, программа вызовет событие `VKWebAppInit` и будет ждать ответа в течение `initWaitTime` миллисекунд (по-умолчанию 1000). Если ответ поступит, произойдёт переключение в `MODE_PROD`, иначе в `MODE_DEV`.
